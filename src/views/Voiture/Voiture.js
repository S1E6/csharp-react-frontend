...
const Voiture = () => {
  const { voitureData, deleteVoiture, records } = useVoitureContext()
  ...
  return (
    <>
      <CRow>
        ...
        <CCol xs={4}>
          <CFormCheck label="Voiture vendue" checked={isSold} onChange={() => setIsSold(!isSold)} />
        </CCol>
        <CCol xs={4}>
          <CFormInput
            type="text"
      ...
      <DataTable
        columns={columns}
        data={searchText.length > 0 ? searchResults : isSold ? records : voitureData}
        fixedHeader
        pagination
        dense={false}
      />
